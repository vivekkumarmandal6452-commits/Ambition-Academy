...
// In-memory fallback store (persists for the lifetime of the serverless function instance)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // ─── FALLBACK ────────────────────────────────────────────────────────────
  return res.status(404).json({ success: false, error: `Route /api/${route} not found`, available: ['/api/health', '/api/auth/login', '/api/auth/register', '/api/auth/me', '/api/batches', '/api/enrollments/me'] });
}
